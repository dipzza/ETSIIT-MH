...
class AG_MDP(object):
    ...
    def fObjetivo(self, dist, sel):
        dist_sel = self.dist[sel[:, None], sel]
    ...
    def fixChromosome(self, chromosome, keep_best=False):
...
df = pd.concat([pd.DataFrame(data_agg_u), pd.DataFrame(data_agg_p), 
               ...
               axis=1)
df.to_csv(r'../resultados_memeticos_fix.csv', index=False)